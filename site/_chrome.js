/* =========================================================
   ...
   • webflow-import/_chrome-source.html shows the canonical
     markup to copy into Webflow's symbol/component.
   ========================================================= */
...
(function () {
  ...
  // Declared on the <script> tag:
  //   data-root=""    → top-level page  (index.html, about.html, …)
  //   data-root=".."  → sub-page        (services/, projects/, journal/)
  const scriptEl = document.currentScript ||
    Array.from(document.scripts).find(s => /_chrome\.js(\?|$)/.test(s.src));
  const root = (scriptEl?.dataset.root ?? '').replace(/\/$/, '');
  const linkPrefix  = root ? root + '/' : '';   // page → site root
  const assetPrefix = root ? root + '/' : '';   // page → site root (assets/ is at site root)
  ...
  const isCurrent = (href) => {
    const target = (linkPrefix + href).replace(/index\.html?$/, '').replace(/^\//, '');
    const norm = here.replace(/^\//, '').replace(/index\.html?$/, '');
    if (!target) return norm === '' || norm === '/';
    const targetBase = target.replace(/\.html$/, '');
    const normBase   = norm.replace(/\.html$/, '');
    return normBase === targetBase || normBase.startsWith(targetBase + '/');
  };
    ...
    <header class="nav" id="site-nav">
      <div class="nav_container">
        <a href="${rel('index.html')}" class="nav_logo" aria-label="BF Property home">
          <img src="${assetPrefix}assets/logo-bf-property-red.png" alt="BF Property" width="108" height="42">
        </a>

        <button class="nav_mobile-toggle" aria-label="Open navigation" aria-expanded="false" aria-controls="primary-nav">
        ...
        <nav aria-label="Primary" class="primary-nav" id="primary-nav">
          <ul class="nav_links" role="list">
            ...
            <li class="has-menu">
              <a href="${rel('services/index.html')}" class="nav_link${current('services/index.html')}" aria-haspopup="true" aria-expanded="false">Services <i data-lucide="chevron-down" class="nav_chevron" aria-hidden="true"></i></a>
            ...
            <li><a href="${rel('projects/index.html')}" class="nav_link${current('projects/index.html')}">Projects</a></li>
            <li><a href="${rel('about.html')}" class="nav_link${current('about.html')}">About</a></li>
            <li><a href="${rel('journal/index.html')}" class="nav_link${current('journal/index.html')}">Journal</a></li>
  ...
  const chromeCss = `
    ...
    @media (prefers-reduced-motion: reduce) { .bf-sticky-cta { transition: none; } }
  `;
  ...
  function inject() {
    ...
    // Mobile toggle
    const toggle = $('.nav_mobile-toggle');
    ...
    // Sticky CTA — show after 600px scroll, dismiss per-session
